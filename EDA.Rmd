...
This document is an exploratory analysis of text data realted to the Capstone Project for the Johns Hopkins Data Science Specialization.

## Intro, data downloading and sapmpling
...
The raw data can be found: [here](url). Our first task is to download the data assuming it was not done before:
...
Because of the data size, the second step is to sample a portion of data. `rbinom` has been used for this purpose sampling 5% of the original data. This sampled data is saved in new txt files:
...
Now we have to load the sampled data and preprocess it. The preprocess step:
* merges the text per language
* remove the punctuation
* remove the numbers
* order the text courpus per word frequency
...
g <- ggplot() + 
    ...
    labs(x='dictionary percentage [%]', y='word frequency [%]') +
